...
if ($activeCartResult->num_rows > 0) {
    ...
    // Set temporary cart ID based on shared_cart or user's own cart ID
    $temporaryCartId = ($sharedCartCode !== null) ? $sharedCartCode : $cartId;
} else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['removeProductId'])) {
    ...
    // Use temporary cart ID in the delete query
...
<body>
    ...
    <div class="container">
        ...
        ?>
        <?php
        $sql = "SELECT * FROM cartitems WHERE cart_id = '$temporaryCartId'"; // Use temporary cart ID
